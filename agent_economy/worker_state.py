...
from agent_economy.schemas import EventType, LedgerEvent
from agent_economy.schemas import WorkerRuntime
...
def default_state_path() -> Path:
    raw = (os.getenv("AE_WORKER_STATE_PATH") or os.getenv("INST_WORKER_STATE_PATH") or "").strip()
    if raw:
        return Path(raw).expanduser()
    preferred = repo_root() / ".beads" / "agent_economy_workers.json"
    legacy = repo_root() / ".beads" / "institution_service_workers.json"
...
def load_state(path: Path) -> PersistedWorkerState:
    if (
        os.getenv("AE_WORKER_STATE_ISOLATION") or os.getenv("INST_WORKER_STATE_ISOLATION") or ""
    ).lower() in (
        "1",
        "true",
        "yes",
