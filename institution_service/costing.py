...
def load_pricing_from_env() -> dict[str, Price]:
    raw = (os.getenv("INST_PRICING_JSON") or "").strip()
    if not raw:
        return {}

    parsed = json.loads(raw)
    if not isinstance(parsed, dict):
        raise ValueError("INST_PRICING_JSON must be a JSON object")
    ...
    for k, v in parsed.items():
        ...
        if isinstance(v, dict):
            ...
            if isinstance(inp, (int, float)) and isinstance(outp, (int, float)):
                ...
                continue
        raise ValueError(
            "INST_PRICING_JSON values must be a number or {input_per_1k, output_per_1k}"
        )
    return out
...
def default_price_for(model_ref: str) -> Price:
    ...
    # Heuristic defaults (unitless). Override with INST_PRICING_JSON for your environment.
    if "mini" in m:
