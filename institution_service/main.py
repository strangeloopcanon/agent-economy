...
from institution_service.config import load_settings, repo_root
from institution_service.config import InstitutionSettings
from institution_service.command_workers import (
    CommandBidder,
    CommandExecutor,
...
from institution_service.openai_bidder import OpenAIBidder
from institution_service.openai_executor import ExecutorSettings, OpenAIExecutor
from institution_service.planner import toposort_plan
from institution_service.planner_workers import decompose_with_worker
from institution_service.scenario import load_scenario
...
from institution_service.workspace_ignore import copytree_ignore, is_ignored_workspace_part
from institution_service.worker_mux import MultiplexBidder, MultiplexExecutor
from institution_service.worker_refs import resolve_worker_ref
from institution_service.worker_specs import CommandWorkerSpec, load_worker_pool_from_json
...
def _default_workers_spec_from_env() -> dict[str, str]:
    ...
    raw = (os.getenv("INST_MODELS_JSON") or os.getenv("ORCH_MODELS_JSON") or "").strip()
    if not raw:
        ...
        if not isinstance(parsed, dict) or any(
            ...
            raise ValueError("INST_MODELS_JSON must be a JSON object of {worker_id: model_ref}")
        models = dict(parsed)
    return models
...
def _load_judge_workers_from_env(
    *, workers: list[WorkerRuntime], command_specs: dict[str, CommandWorkerSpec]
    ...
    raw = (os.getenv("INST_JUDGES_JSON") or os.getenv("INST_JUDGE_WORKERS_JSON") or "").strip()
    if raw:
        parsed = json.loads(raw)
        ...
        raise ValueError(
            "INST_JUDGES_JSON/INST_JUDGE_WORKERS_JSON must be a JSON list[str] or object of {name: worker_ref}"
        )
...
def _planner_worker_ref_from_env(
    *, workers: list[WorkerRuntime], command_specs: dict[str, CommandWorkerSpec]
    ...
    raw = (os.getenv("INST_PLANNER_WORKER") or os.getenv("INST_PLANNER_MODEL") or "").strip()
    if raw:
        return raw
    for w in workers:
        if w.worker_type == WorkerType.MODEL_AGENT and w.model_ref:
        ...
        if w.worker_type == WorkerType.EXTERNAL_WORKER:
            ...
            if spec is not None and bool(spec.plan_cmd):
                return w.worker_id
    raise ValueError(
        "no planner worker available (set INST_PLANNER_WORKER/INST_PLANNER_MODEL or provide a planner-capable worker)"
    )
...
def _timeout_seconds_from_env(*, env_var: str, default: float | None) -> float | None:
    import os
...
def _engine_settings(*, max_concurrency: int) -> EngineSettings:
    return EngineSettings(
        ...
        bid_timeout_seconds=_timeout_seconds_from_env(
            env_var="INST_BID_TIMEOUT_SECONDS",
            default=30.0,
        ),
        execution_timeout_seconds=_timeout_seconds_from_env(
            env_var="INST_EXECUTION_TIMEOUT_SECONDS",
            default=300.0,
        ),
    )
...
def _print_next_steps(*, run_dir: Path, workspace_dir: Path, state) -> None:
    ...
    print(f"Full report: institution-service report --run-dir {run_dir}")
    if state.tasks and any(t.status == "REVIEW" for t in state.tasks.values()):
        print(f"Manual review pending: institution-service review list --run-dir {run_dir}")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="institution_service")
    ...
    def _add_single_task_args(p: argparse.ArgumentParser, *, default_decompose: bool) -> None:
        ...
        p.add_argument(
            ...
            type=str,
            default=None,
            help="planner worker ref (default: INST_PLANNER_WORKER/INST_PLANNER_MODEL or first capable worker)",
        )
        p.add_argument(
            "--judge-worker",
            "--judge-model",
    ...
    if args.cmd == "dashboard":
        from institution_service.dashboard.server import run_dashboard
    ...
    if args.cmd == "run":
        ...
        if needs_judges:
            ...
            if needs_default_judges and not judge_workers:
                raise SystemExit("judge workers required (set INST_JUDGES_JSON)")
    ...
    if args.cmd in {"oneshot", "task"}:
        ...
        if needs_judges and not judge_workers:
            raise SystemExit("judge workers required (use --judge-worker or set INST_JUDGES_JSON)")
        ...
        if args.decompose:
            ...
            if planner_ref:
                ...
                env_ref = (
                    os.getenv("INST_PLANNER_WORKER") or os.getenv("INST_PLANNER_MODEL") or ""
                ).strip()
                if env_ref:
                    planner_ref = env_ref
